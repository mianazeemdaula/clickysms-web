...
use App\Helpers\SMSActivateErrorCodes;
...
class SMSActivate
{
    ...
    private function request($data, $method, $parseAsJSON = null, $getNumber = null)
    {
        ...
        $responseError = new SMSActivateErrorCodes($result);
        $check = $responseError->checkExist($result);

        try {
            if ($check) {
                throw new SMSActivateRequestError($result);
            }
        } catch (Exception $e) {
